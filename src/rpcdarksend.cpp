...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <Olympicaddress> <amount>\n"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  stop         - Stop masternode configured in Olympic.conf\n"
                ...
                "  vote-many    - Vote on a Olympic initiative\n"
                "  vote         - Vote on a Olympic initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return COlympiccoinSecret(secret).ToString();
    }
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a Olympic initiative (can be additionally filtered, partial match)\n"
