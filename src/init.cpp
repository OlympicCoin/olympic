...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Olympic.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Olympicd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeOlympicamount=<n> " + _("Keep N Olympic anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Olympic is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Olympic is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Olympic version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Olympic server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Olympic") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Olympic to complete") << "\n";
    ...
    nAnonymizeOlympicAmount = GetArg("-anonymizeOlympicamount", 0);
    if(nAnonymizeOlympicAmount > 999999) nAnonymizeOlympicAmount = 999999;
    if(nAnonymizeOlympicAmount < 2) nAnonymizeOlympicAmount = 2;
    ...
    LogPrintf("Anonymize Olympic Amount %d\n", nAnonymizeOlympicAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1OLMP+1000 == (.1OLMP+100)*10
       10OLMP+10000 == (1OLMP+1000)*10
    */
