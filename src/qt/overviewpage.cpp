...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeOlympicAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeOlympicAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeOlympicAmount = strAnonymizeOlympicAmount.remove(strAnonymizeOlympicAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeOlympicAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeOlympicAmount*COIN) nMaxToAnonymize = nAnonymizeOlympicAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeOlympicAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeOlympicAmount));
        strAnonymizeOlympicAmount = strAnonymizeOlympicAmount.remove(strAnonymizeOlympicAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeOlympicAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
