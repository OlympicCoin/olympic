...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("Olympic://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Olympic:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Olympic.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Olympic.lnk
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Olympic app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Olympic app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Olympic-Qt") + " " + tr("version") + " " +
