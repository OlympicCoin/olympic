...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeOlympicAmount"))
        settings.setValue("nAnonymizeOlympicAmount", 1000);
    nAnonymizeOlympicAmount = settings.value("nAnonymizeOlympicAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeOlympicAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeOlympicAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeOlympicAmount:
            return QVariant(nAnonymizeOlympicAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeOlympicAmount:
            nAnonymizeOlympicAmount = value.toInt();
            settings.setValue("nAnonymizeOlympicAmount", nAnonymizeOlympicAmount);
            emit AnonymizeOlympicAmountChanged(nAnonymizeOlympicAmount);
            break;
        default:
            break;
        }
